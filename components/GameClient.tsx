...
type View = {
  code: string; phase: "waiting" | "generating" | "predicting" | "answering" | "scoring" | "reveal"; currentRound: number;
  me: Player; players: Player[]; round?: { number: number; subject: string; predictor: string; questions: Question[] };
...
type LandingSession = {
  displayName: string;
  games: Array<{ code: string; phase: View["phase"]; currentRound: number; role: Player["role"] }>;
};
...
function JoinForm({ code, onDone }: { code: string; onDone: (code: string) => void }) {
  const [error, setError] = useState("");
  const [busy, setBusy] = useState(false);
  async function submit(event: FormEvent) {
    event.preventDefault(); setError(""); setBusy(true);
    try {
      const result = await post(`/api/games/${code}/join`);
      onDone(result.code);
    } catch (e) { setError(e instanceof Error ? e.message : "Please try again."); } finally { setBusy(false); }
  }
  return <form onSubmit={submit} className="space-y-4">
    <ErrorText error={error} /><Button type="submit" disabled={busy}>{busy ? "One sec…" : "Join game →"}</Button>
  </form>;
}
function Landing({ session }: { session?: LandingSession }) {
  ...
  async function create() {
    ...
    try {
      const result = await post("/api/games");
  ...
  async function logout() {
    ...
    try {
      ...
      router.push("/");
      router.refresh();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to log out."); } finally { setBusy(false); }
  }
  return <Shell><section className="mt-10 space-y-8">
    <div><h1 className="text-5xl font-black leading-[.95] tracking-[-.06em] sm:text-6xl">Do you know<br /><span className="text-purple">me-me?</span></h1><p className="mt-5 max-w-sm text-lg font-semibold leading-relaxed text-ink/65">Predict what your favorite person would pick. Then find out if you nailed it.</p></div>
    {!session ? <div className="space-y-4"><a href="/signup" className="block w-full rounded-2xl bg-ink px-5 py-4 text-center font-extrabold text-white shadow-lg shadow-ink/15 transition hover:-translate-y-0.5">Sign up to play</a><a href="/login" className="block w-full rounded-2xl border-2 border-ink/10 bg-white px-5 py-4 text-center font-extrabold">Log in</a><p className="text-center text-xs font-bold text-ink/35">Your games and scores stay with your account.</p></div> :
      <><div className="flex items-center justify-between"><div><p className="text-sm font-bold text-ink/50">Playing as</p><p className="text-xl font-black">{session.displayName}</p></div><button onClick={() => void logout()} disabled={busy} className="text-sm font-extrabold text-purple disabled:opacity-45">Log out</button></div>
      <ErrorText error={error} />
      {!joining ? <div className="space-y-4"><Button disabled={busy} onClick={() => void create()}>{busy ? "Creating…" : "Create a game ✨"}</Button><button onClick={() => setJoining(true)} className="w-full rounded-2xl border-2 border-ink/10 bg-white px-5 py-4 font-extrabold">Join with a game code</button></div> :
        <div className="space-y-4 rounded-3xl bg-white p-5 shadow-card"><label className="block text-sm font-extrabold">Game code<input value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase().replace(/[^A-Z2-9]/g, "").slice(0, 6))} placeholder="ABC123" maxLength={6} className="mt-2 w-full rounded-2xl border-2 border-black/5 px-4 py-4 text-center text-2xl font-black tracking-[.3em] outline-none focus:ring-4 focus:ring-purple/20" /></label>{joinCode.length === 6 && <JoinForm code={joinCode} onDone={(code) => router.push(`/game/${code}`)} />}<button onClick={() => setJoining(false)} className="w-full text-sm font-bold text-ink/50">← Back</button></div>}
      <section className="space-y-3"><h2 className="text-2xl font-black">My games</h2>{session.games.length ? <div className="space-y-3">{session.games.map((game) => <a key={game.code} href={`/game/${game.code}`} className="flex items-center justify-between rounded-3xl bg-white p-5 shadow-card transition hover:-translate-y-0.5"><div><p className="font-black">Game {game.code}</p><p className="mt-1 text-sm font-semibold text-ink/55">{game.currentRound ? `Round ${game.currentRound} · ${game.phase}` : "Waiting for a second player"}</p></div><span className="font-extrabold text-purple">Continue →</span></a>)}</div> : <div className="rounded-3xl bg-white p-5 font-semibold text-ink/60 shadow-card">Create a game or join a friend to see it here.</div>}</section></>}
  </section></Shell>;
}
...
function GameRoom({ code }: { code: string }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        const message = data.error || "Unable to load game.";
        setCanJoin(response.status === 403 && message === "Join this game before playing.");
  ...
  if (error && !view) return <Shell code={code}><ErrorText error={error} />{canJoin ? <section className="mt-4 rounded-3xl bg-white p-6 text-center shadow-card"><p className="mb-5 text-2xl font-black">You’ve been invited! 🎟️</p><JoinForm code={code} onDone={() => void load()} /></section> : <button onClick={() => router.push("/")} className="mt-4 font-bold underline">Back home</button>}</Shell>;
  if (!view) return null;
  ...
  return <Shell code={code}><div className="mb-5 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-widest text-ink/40">You are</p><p className="font-black">{view.me.name}</p></div><div className="text-right"><p className="text-xs font-black uppercase tracking-widest text-ink/40">Score</p><p className="font-black text-purple">{view.me.score}</p></div></div><ErrorText error={error} />
    ...
    {view.phase === "generating" && <WaitingCard title="Preparing your questions…" body="This only takes a moment. Keep this page open while we set up the round." />}
    ...
    {view.phase === "scoring" && <WaitingCard title="Tallying the reveal…" body="Your answers stay private until the results are ready." />}
...
export default function GameClient({ initialCode, session }: { initialCode?: string; session?: LandingSession }) {
  ...
  return code ? <GameRoom code={code} /> : <Landing session={session} />;
}
